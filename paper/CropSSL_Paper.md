...
4. **Cross-domain evaluation** — accuracy, macro-F1, expected calibration error (ECE), false-discovery rate, Grad-CAM, embedding visualizations, and CKA representation-similarity analysis — across thirteen registered datasets spanning lab and field conditions.
...
- **VICReg** (shipped, not benchmarked in this paper) trains on two augmented views like SimCLR but replaces negative pairs with explicit variance and covariance regularizers that prevent representation collapse. It is wired into the same factory, training scripts, and benchmark harness; the experiments reported here were run before it was added.
...
We report numbers on thirteen registered datasets, but the three that carry the argument are:
...
The framework we built to test this — four SSL methods on a shared backbone (VICReg ships as a fifth but postdates these experiments), four adaptation strategies, optional domain alignment, and a thirteen-dataset evaluation harness — is open source and reproducible. If this paper nudges one applied group toward pre-training before fine-tuning, or toward measuring calibration alongside accuracy, it has done its job.
